...
import time
import re
from YDMHTTPDemo3 import checkcaption
...
#如果接收数据是这样的格式
getData1 = {'from_station':'成都','to_station':'上海','train_date':'2019-04-01','back_traib_date':'2019-04-01',}
getData2 = {'train_station':'D2208'}
getData3 = {'passengerInfo':[{'passengerName':'周常青','IDCard':'510902199701146057','seatType':'一等座','pasType':'成人票'},{'passengerName':'唐亿','IDCard':'510902199708158472','seatType':'二等座','pasType':'学生票'}]}
...
class buyTickets():
    def __init__(self):
        # 保存 train_no 的值，通过字典形式。key = 列车号，values = train_no
        self.train_no = {}
        # 保存 secretStr 的值，通过字典形式。key = 列车号，values = secretStr
        self.secretStr = {}
        # 保存 leftTicket 的值，通过字典形式。key = 列车号，values = leftTicket
        self.leftTicket = {}
        # 保存 购买火车票时：用户的信息
        self.passenger = {}
        # 保存 购买火车票时：列车信息
        self.train_location = {}
        # 用于后面的请求页面，保存 cookie 值。
        self.session = requests.session()
    def login(self):
        # 在登录时 需要先获取验证码，再通过 post 请求将验证码的答案提交上去
        # 这里需要获取验证码
        ...
        if int(content['result_code']) == 4:
            ...
            return
        # 如果我们不想定死可以改成 input 输入形式
        name = account.user12306
        password = account.pwd12306
    ...
    def query(self):
        ...
        data = {'leftTicketDTO.train_date': getData1['train_date'],
                ...
                'purpose_codes': 'ADULT'
                }#purpose_codes  ADULT -- 普通  0X00 -- 学生
        # 将 data 格式化
        ...
        # 在 getData2['train_station'] 中为需要购买的列车号，因为是自动了所以就固定死了，也可以修改
        # getData2['train_station'] = input('请输入需要购买的列车号')

        # # 查看列车经过的站台
        # url = 'https://kyfw.12306.cn/otn/czxx/queryByTrainNo?'
        # data = {
        #     'train_no': self.train_no[string[1]],
        #     'from_station_telecode': string[2],
        #     'to_station_telecode': string[3],
        #     'depart_date': string[11][:4] + "-" + string[11][4:6] + "-" + string[11][6:8]
        # }
        # # 将data转码
        # data = urllib.parse.urlencode(data)
        # # 拼接 url
    ...
    def buy(self):
        ...
        # 获取购买车票的信息 也可以购买多张车票。如果需要改动可以将下面的注释解除
